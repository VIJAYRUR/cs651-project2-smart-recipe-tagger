...
const PhotoGallery = () => {
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    const ai = recipe.aiAnalysis || {};

    // Search term filter
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase();
      const title = (ai.title || recipe.filename || '').toLowerCase();
      const recipeTags = (ai.tags || []).join(' ').toLowerCase();
      if (!title.includes(term) && !recipeTags.includes(term)) {
        return false;
      }
    }
    ...
    if (filters.cookingTime.length > 0) {
      const totalTime = ai.totalTimeMinutes ||
        ((ai.prepTimeMinutes || 0) + (ai.cookTimeMinutes || 0));

      let matchesTime = false;
      if (filters.cookingTime.includes('quick') && totalTime > 0 && totalTime <= 30) {
        matchesTime = true;
      }
      if (filters.cookingTime.includes('medium') && totalTime > 30 && totalTime <= 60) {
        matchesTime = true;
      }
      if (filters.cookingTime.includes('long') && totalTime > 60) {
        matchesTime = true;
      }
    ...
    if (filters.calories.length > 0) {
      const calories = ai.nutritionPerServing?.calories || 0;

      let matchesCalories = false;
      if (filters.calories.includes('low') && calories > 0 && calories < 300) {
        matchesCalories = true;
      }
      if (filters.calories.includes('medium') && calories >= 300 && calories <= 600) {
        matchesCalories = true;
      }
      if (filters.calories.includes('high') && calories > 600) {
        matchesCalories = true;
      }
      if (!matchesCalories) return false;
    }

    // Allergen filter (dairy-free, gluten-free, etc.)
    if (filters.allergens.length > 0) {
      const recipeAllergens = (ai.allergens || []).map(a => a.toLowerCase());

      for (const allergenFilter of filters.allergens) {
        if (allergenFilter === 'dairy-free' && recipeAllergens.some(a => a.includes('dairy') || a.includes('milk'))) {
          return false;
        }
        if (allergenFilter === 'gluten-free' && recipeAllergens.some(a => a.includes('gluten') || a.includes('wheat'))) {
          return false;
        }
        if (allergenFilter === 'nut-free' && recipeAllergens.some(a => a.includes('nut') || a.includes('peanut'))) {
          return false;
        }
        if (allergenFilter === 'egg-free' && recipeAllergens.some(a => a.includes('egg'))) {
          return false;
        }
      }
    }
    ...
    if (filters.tags.length > 0) {
      const recipeTags = (ai.tags || []).map(t => t.toLowerCase());
      const hasMatchingTag = filters.tags.some(filterTag =>
        recipeTags.includes(filterTag.toLowerCase())
      );
